...
def test_create_post_default_published_true(authorized_client, test_posts, test_user):
    res = authorized_client.post('/posts/', json={'title': 'arbitrary title', 'content': 'Nothing'})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == 'arbitrary title'
    assert created_post.content == 'Nothing'
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']


def test_unauthorized_user_create_post(client, test_posts):
    res = client.post('/posts/', json={'title': 'arbitrary title', 'content': 'Nothing'})
    assert res.status_code == 401
...
def test_delete_other_user_post(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.delete(f'/posts/{test_posts[3].id}', json=data)
    assert res.status_code == 403
